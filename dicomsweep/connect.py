"""Native cognis-connect emit for dicomsweep — forward findings to any platform.

Maps dicomsweep's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    dicomsweep ... --format json | dicomsweep-emit --to stix
    dicomsweep-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out.pop('vr', None)
        out.pop('action', None)
        out.pop('current_value', None)
        out['title'] = rec.get('tag_hex', '') + ' ' + rec.get('keyword', '')
        out['severity'] = 'medium'
        out['type'] = 'dicom_tag'
        out['description'] = rec.get('current_value', '')
        out['tags'] = [rec.get('tag_hex', ''), rec.get('keyword', '')]
