...
**dicomsweep** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `dicomsweep` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
dicomsweep ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
dicomsweep ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
dicomsweep ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `dicomsweep` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="dicomsweep") for rec in records]   # records = your JSON output
